...
export function pageTable(objArray: any[], pagination: Pagination): any[] {
  if(objArray && objArray.length && pagination) {
    ...
    console.log(first)
    let last: number = (pagination.currentPage * pagination.numberPerPage) - 1;
    console.log(last)
...
export function setPaging(objArrLen: number, pagination: Pagination): Pagination {
  if(objArrLen > 0) {
    ...
    pagination['currentPage'] = pagination.currentPage && pagination.currentPage > 1 ? pagination.currentPage - 1 : 1;
